...
def build_yref_online(y_idx, ref_vec):
    ...
    yref[y_idx["quat"]]    = np.array([1,0,0,0])   # mutual rotation error as quaternion (mutual des rot = unit quat)
...
def set_initial_state(ocp_solver, xk):
    '''
    Utile per fissare le condizioni iniziali dell'MPC come stato corrente ad ogni iterazione
    '''
    # vincolo di uguaglianza x(0)=xk: lbx_0 = ubx_0 = xk
    ocp_solver.set(0, "lbx", xk)
    ocp_solver.set(0, "ubx", xk)

def configure_mpc(model, x0, p_obj, rpy_obj, Tf, ts, W, W_e, ref = np.zeros(6), final_ref = np.zeros(6)):
    ...
    # Control constraints
    Fmax = 4*9.8*m  #more or less 4 times than hovering
    Tmax = [0.25, 0.25, 0.15]
...
############################################################################################################
    ...
    mutual_R = R_drone.T @ R_obj  # mutua rotazione
    ...
    ocp.cost.W_e = W_e
    ocp.cost.set = True
    # initial values of parametes (p_obj,rpy_obj, mut_rot_ref) (they will be updated at each iteration )
    ...
    ny   = y_idx["u"].stop   # lunghezza di y (include u)
    ny_e = y_idx["u"].start  # lunghezza di y_e (esclude u)
